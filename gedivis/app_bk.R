...
pchIcons(pch=shapesPt, width=40,height=40,col=c('blue','blue'),lwd=4)
...
if(file.exists(paste0(outDir, '/xleafSim.RData'))){
  ...
  heigthPal <<-  colorNumeric( #reverse = TRUE,
                              (c('yellow', "gold", 'orange', 'green', "darkgreen")) ,
                         #c("#0C2C84", "#41B6C4", "#FFFFCC"), 
                         domain = fagua1kmVals,
                         na.color = "transparent")
  
  leafFagua <<- leaflet() %>% addTiles() %>% 
    addRasterImage(fagua1kmR, colors = heigthPal, opacity = 1, group = "Heigth") %>%
    ...
    addPolygons(data = orbits, color = 'red', weight = 0, 
                opacity = .9,fillOpacity = .2, group = 'Orbits',
                label = ~htmlEscape(StrtOrN)) %>%
    addLayersControl(position = 'topleft',
                       overlayGroups = c('Heigth', 'Orbits'),
                     options = layersControlOptions(collapsed = FALSE),
                     baseGroups = c("OpenStreetMap", "Esri.WorldImagery")) %>%
    addProviderTiles( "Esri.WorldImagery", group = "Esri.WorldImagery" ) #%>% hideGroup(c('Heigth', 'Orbits'))
  ...
  dashboardSidebar(disable = FALSE,
                   sidebarMenu(
                     ...
                     menuItem("Similarity", tabName = "tab_similarity", startExpanded = TRUE,
                              menuSubItem("How it works?", tabName = "tab_worksimi"),
    ...
    tabItems(
      ...
      tabItem('tab_gedi2', 
              ...
              fluidRow(#width = 6, status = "info", solidHeader = TRUE, title = "Title", height = 500,
                ...
                column( width = 6,
                        fluidRow( 
                          #h5('Click the biotic regions map >> click an ecosystem >> click up to 3 points >> Clear'),
                          column( width = 5, h5('1. Select an ecorregion in the map'), 
              ...
              fluidRow(#width = 6, status = "info", solidHeader = TRUE, title = "Title", height = 500,
                ...
                column( width = 6,
                        fluidRow( 
                          ...
                          column( width = 2, h5('2. Click regions in the map'), 
      ...
      tabItem("tab_similarity", 
              ...
              fluidRow(
                ...
                column(width = 6, 
                       fluidRow(column(width = 4, selectInput("in_r", "Pts color: Red", xyDfCNr, selected = 'u1')),
              ...
              fillPage(
               ...
               #tags$style(type = "text/css", "html, body {width:100%;height:100%}"),
                ...
                absolutePanel(top = 50, right = 10,
                              actionButton("go_height", "Clip map"),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$mapx_click, {
    ...
    #save(click, file = 'mapxClick.RData')
    #load('mapxClick.RData')
    buffPt <- rgeos::gBuffer(sp::SpatialPoints(cbind(click3$lng, click3$lat)),
                             width = 0.0083/1)
    buffPt <<- SpatialPolygonsDataFrame(buffPt, data.frame(a = 1,
                                                           x = click3$lng, 
                                                           y = click3$lat
                                                           ), match.ID = FALSE)
  ...
  observeEvent(input$go_height, {
    if(exists('buffPt')){
      ...
      suppressWarnings(rast <- gdalUtilities::gdalwarp(srcfile = paste0(rdatPath, '/', fagua30m),
                                                       dstfile = tempRaster,
      ...
      proxy %>% 
        addRasterImage(hmap, group = 'Crop',
                       colors = heigthPal, opacity = 1)  %>%
        addCircleMarkers(data = mongoResult, lng = ~lon_lm_a0, lat = ~lat_lm_a0,
                         # https://stackoverflow.com/questions/43144596/r-and-leaflet-how-to-arrange-label-text-across-multiple-lines
      ...
      output$rglPlot <- renderRglwidget({
        ...
        if(nrow(mongoResult) > 0){
          ...
          points3d(mongoResult$lon_lm_a0,
                   mongoResult$lat_lm_a0,
                   mongoResult$rh_100_a0, 
                   col = 2
                   #col = '#E0E318',radius = 0.1, lit = TRUE
          )
        }
      ...
      if(myClickPol$group == 'Ecosystems' & myClickPol$id != ecoID){
        ...
        output$mapleaf <- renderLeaflet({ 
          ...
          gediLabel <- paste0('<strong>Point ID:', shp_rh_small$idsmall,'</strong>',
                              ...
                              '<strong>ShotNum:', shp_rh_small$shotNum,'</strong>',
                              '<br>Max. height: ', round(shp_rh_small$mx_h, 3),
                              '<br>Beam: ', shp_rh_small$beam,
                              '<br>Date: ', shp_rh_small$date,
                              '<br>Day of year: ', shp_rh_small$DOY,
        ...
        output$plotcurve <- renderHighchart({
          load(paste0(rdatPath, '/rdat_2a/hc_', ecoID,'.RData')) # 
          hc0 <<- hc
          hc <<- hc
          hc
        })
    ...
    if(is.numeric(myClickPol2$id) & is.character(myClickPol2$group)){
      ...
      if(myClickPol2$group == 'Ecosystems' & myClickPol2$id != ecoID){
        ...
        output$mapleaf2 <- renderLeaflet({ 
          ...
          gediLabel <- paste0('<strong>Point ID:', shp_rh_small$idsmall,'</strong>',
                              '<br>Ecosys. ID:', ecoID,'</strong>',
                              '<strong>ShotNum:', shp_rh_small$shot_num,'</strong>',
                              '<br>Max. height: ', round(shp_rh_small$mx_h, 3),
                              '<br>Beam: ', shp_rh_small$beam,
                              '<br>Date: ', shp_rh_small$date,
                              '<br>Day of year: ', shp_rh_small$doy,
        ...
        output$plotcurve2 <- renderHighchart({
          ...
          if (!file.exists(paste0(rdatPath, '/rdat_2b/md_curve_', ecoID,'.RData'))){
            ...
            if( input$eneSelected == 'Proportional'){
              me_curve$xAxis <- me_curve$me3x
              me_curve$xAxisAv <- me_curve$av3x
...
#  input <- list(xAxisSlider = 100, yAxisSlider = 100, eneSelected = 'Absolute', varSelected = 'PAI')
  ...
  # observeEvent(input$yAxisSlider,{
  #   if(exists('myClickPol')){
  #     output$plotcurve2 <- renderHighchart({
  ...
  # })
  # 
  # observeEvent(input$xAxisSlider,{
  #   if(exists('myClickPol')){
  #     output$plotcurve2 <- renderHighchart({
  #       hc <<- hc %>% 
  ...
  observeEvent(input$varSelected3,{
    # ecoID <- 139
    if (!file.exists(paste0(rdatPath, '/rdat_2b/md_curve_', ecoID,'.RData'))){
      highEmpty
    } else {
      ...
      updateSliderInput(session, inputId = "yAxisSlider3", 
                        value = max(me_curve$rh, na.rm = TRUE),
                        min = 0, max = max(me_curve$rh, na.rm = TRUE), step = stepSlider)
      
      updateSliderInput(session, inputId = "xAxisSlider3", 
                        value = max(me_curve$xAxis, na.rm = TRUE),
                        min = 0, max = max(me_curve$xAxis, na.rm = TRUE), step = stepSlider)
  ...
  observeEvent(input$eneSelected,{
    if (!file.exists(paste0(rdatPath, '/rdat_2b/md_curve_', ecoID,'.RData'))){
      highEmpty
    } else {
      load(paste0(rdatPath, '/rdat_2b/md_curve_', ecoID,'.RData')) # me_pavd; me_cov; me_pai; me
  ...
  observeEvent(input$eneSelected3,{
    if (!file.exists(paste0(rdatPath, '/rdat_2b/md_curve_', ecoID,'.RData'))){
      highEmpty
    } else {
      load(paste0(rdatPath, '/rdat_2b/md_curve_', ecoID,'.RData')) # me_pavd; me_cov; me_pai; me
    ...
    if(!is.null(myClickPol3$id)){
      ...
      output$plotcurve3 <- renderHighchart({
        ...
        if (!file.exists(paste0(rdatPath, '/rdat_2b/md_curve_', ecoID3,'.RData'))){
          ...
          xmx <- max(c(max(c(me_curve$xAxisAv, me_curve$xAxis),
                             na.rm = TRUE), max(hc3$x$hc_opts$xAxis$max, na.rm = TRUE)))
  ...
  output$plot <- renderPlotly({
    ...
    output$leafPts <- renderLeaflet({
      ...
      iconFiles <- pchIcons(pch = 16 + (xyDf$isPnn), width = 10, height = 10, col = (ptsLLCol))
...
# sudo rm /home/shiny/tmpR/leafSim.RData
